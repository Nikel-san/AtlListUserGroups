...
def process_user(base_url, auth, user_email, dest_email=None):
    try:
    ...
    if not account_id:
        print(f"No user found matching email: {user_email}", file=sys.stderr)
        return
    ...
    if not groups:
        print(f"No groups found for user {user_email} (accountId {account_id})")
        return
    ...
    if dest_email:
        ...
        if not dest_account_id:
            print(f"No destination user found matching email: {dest_email}", file=sys.stderr)
            return
...
def main():
    ...
    p.add_argument("-u", "--user-email", required=False, help="Target user's full email to query groups for (e.g. target@example.com)")
    ...
    p.add_argument("-f", "--file", help="CSV file containing source and destination email pairs")
    args = p.parse_args()

    if not args.file and not args.user_email:
        p.error("one of --user-email or --file is required")
    if args.file and args.dest_email:
        p.error("--dest-email cannot be used with --file")
    ...
    if args.file:
        try:
            with open(args.file, newline='', encoding='utf-8') as csv_file:
                for row_number, row in enumerate(csv.reader(csv_file), start=1):
                    if not row or not any(cell.strip() for cell in row):
                        continue
                    if len(row) < 2 or not row[0].strip() or not row[1].strip():
                        print(f"Skipping CSV row {row_number}: expected source and destination emails", file=sys.stderr)
                        continue
                    process_user(base_url, auth, row[0].strip(), row[1].strip())
        except OSError as e:
            print(f"Error reading CSV file '{args.file}': {e}", file=sys.stderr)
            sys.exit(2)
    else:
        process_user(base_url, auth, args.user_email, args.dest_email)
